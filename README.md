...
主分支仅保留本地文件处理工具集 `file_tools/`；已弃置的功能（上海地铁票价查询）
存档在 `archive` 分支（该分支仅存档此项，不含 file_tools 历史版本），不在主分支维护。
...
图形界面为深色分组侧边栏（图片工具 / 媒体工具 / 文件与小说，功能入口统一「对象+动作」定名）+ 卡片表单布局（小窗口下表单可滚动），已做高 DPI 适配（高分屏不模糊），路径一律通过资源管理器对话框选择，执行过程与结果汇总显示在底部日志区。窗口在完成构建与定位后一次性显示，启动无闪烁。三种启动方式：
...
- 直连失败（连接被重置/站点反爬）时按提示改用「浏览器模式」：打开本机浏览器
  （独立临时配置）播放视频并捕获媒体地址（含完整请求头），适合被网络阻断的站点；
  下载经多级传输回退（requests → curl_cffi 指纹 → SNI 精简 → 浏览器引擎）。
...
- 可视化界面「番茄小说搜索下载」页：搜索 → 双击选中 → 选格式/范围 → 下载。
